...
class Logger(logging.LoggerAdapter):
    ...
    def __init__(self, module_name, test_out_dir, testcase_id):
        ...
        logr.addHandler(fh)
        super().__init__(logr, {'logger_name': module_name, 'testcase_id': testcase_id})
    ...
    def logMessage(self, msg, level):
        logrec = LogEntry(self.extra['testcase_id'], self.extra['logger_name'], level, msg)
        return json.dumps(logrec.__dict__, default=str)

class LogEntry:

    def __init__(self, testcase_id, logger, level, msg):
