...
class path_selection_L2_L4_steering(TestBase):
    ...
    def run(self):
        ...
        #########################
        deviceClient_Head = stc.device_config(Port=portHandleClient, IfStack=configGLobal.ifStackDevice, IfCount=configGLobal.ifCountDevice, Name=configClientPort.nameDevice1, Address=configClientPort.ipDevice1,Gateway=configClientPort.gatewayDevice1)
        deviceClient_Device1 = stc.device_config(Port=portHandleClient, IfStack=configGLobal.ifStackDevice, DeviceCount=configClientPort.countDevice2, IfCount=configGLobal.ifCountDevice, Name=configClientPort.nameDevice2,Address=configClientPort.ipDevice2)
        deviceClient_Device2 = stc.device_config(Port=portHandleClient, IfStack=configGLobal.ifStackDevice, DeviceCount=configClientPort.countDevice3, IfCount=configGLobal.ifCountDevice, Name=configClientPort.nameDevice3,Address=configClientPort.ipDevice3)
        deviceMpls_Head = stc.device_config(Port=portHandleMpls, IfStack=configGLobal.ifStackDevice, IfCount=configGLobal.ifCountDevice, Name=configMplsPort.nameDevice1,Address=configMplsPort.ipDevice1,Gateway=configMplsPort.gatewayDevice1)
        deviceMpls_Device1 = stc.device_config(Port=portHandleMpls, IfStack=configGLobal.ifStackDevice, DeviceCount=configMplsPort.countDevice2, IfCount=configGLobal.ifCountDevice, Name=configMplsPort.nameDevice2,Address=configMplsPort.ipDevice2)
        deviceInternet_Head = stc.device_config(Port=portHandleInternet, IfStack=configGLobal.ifStackDevice, IfCount=configGLobal.ifCountDevice, Name=configInternetPort.nameDevice1,Address=configInternetPort.ipDevice1,Gateway=configInternetPort.gatewayDevice1)
        deviceInternet_Device1 = stc.device_config(Port=portHandleInternet, IfStack=configGLobal.ifStackDevice, DeviceCount=configInternetPort.countDevice2, IfCount=configGLobal.ifCountDevice, Name=configInternetPort.nameDevice2,Address=configInternetPort.ipDevice2)
        ...
        tx_result_data_dict = {}
        rx_result_data_dict = {}
        captureHndCurrent = stc.get(portHandleClient, 'children-capture')
        ...
        stc.perform("CaptureStart", {'CaptureProxyId': captureHndCurrent})
        time.sleep(2)
        ...
        time.sleep(62)
        logger.info("Stop capture and save all captured files.")
        stc.perform("CaptureStop", {'CaptureProxyId': captureHndCurrent})
        time.sleep(5)
        stc.perform('CaptureDataSave', {'CaptureProxyId': captureHndCurrent, 'FileName': test_case_name +".pcap",\
                                        'FileNameFormat': 'PCAP'})
        time.sleep(5)
        ...
        if rx_mpls_count == mpls_stream_frames:
            ...
            raise RuntimeError("Mpls port can't receieve all ports which send by client port. \
        ...
        if rx_inet_count == inet_stream_frames:
            ...
            raise RuntimeError("Inet port can't receieve all ports which send by client port.")
        ...
        if frames_loss_count > 0:
            ...
            logger.info("No frame loss for all stream blocks.")
